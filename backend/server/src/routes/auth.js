  ...
  try {
    ...
    if (!mailer) {
      return res.status(503).json({
        message: 'Email verification is not configured on the server yet.',
      })
    }
...
router.post('/signup/verify-otp', async (req, res, next) => {
  try {
    ...
    const pending = await SignupVerification
      .findOne({ email })
...
router.post('/password-reset/verify-otp', async (req, res, next) => {
  try {
    ...
    if (otpHash !== reset.otpHash) {
      ...
      return res.status(400).json({
        message: 'That reset code is incorrect.',
      })
    }
...
router.patch('/password', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!user?.passwordHash) {
      return res.status(400).json({
        message:
          'This account does not have a password. Sign in with Google instead.',
      })
    }
